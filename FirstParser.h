...
//std::vector<int> id_1000{ 4,3,/*12,*/13,14,15,16,20,21 };	// Общая информация
//std::vector<int> id_2052{ 5,6,7,8 };					// Система
//std::vector<int> id_2053{ 9,10 };						// Системная плата
//std::vector<int> id_2055{ 12 };							// Процессоры ?????? (Вычислить кол-во процессоров)
//std::vector<int> id_2056{ 17,18,19 };					// Устройства памяти (оперативка)
//std::vector<int> id_2081{ 22,23,24,25 };				// АТА (жесткие диски, кол-во через запятую)
//std::vector<int> id_2042{ 31,32 };						// Видео Windows (видеокарты)
//std::vector<int> id_2041{ 33,34,35,36 };				// Монитор (может вместо строки "Модель", взять после "<tr><td class='h'>" ?)
//std::vector<int> id_2201{ 26,27,28,29,30 };				// Сетевые адаптеры
//std::vector<int> id_2350{ 37 };							// Пользователи (нужно перечислить не отключенных)
//std::vector<int> id_2311{ 38 };							// Активные сессии (перечислить активные сессии)
